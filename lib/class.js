...
export class HttpClient {
  ...
  async health () {
    const health = () => this._health(this, this.healthPath, this.healthBaseUrl)
    return health()
  }
  ...
  async _got (...gotArgs) {
    ...
    try {
      log.info(`${name}: Start`)
      const got = this._createGot(args, log)
      ...
      log.error({ err, statusCode: err.statusCode }, `${name}: Fail`)
      throw err
    }
  }
  ...
  _createGot ({ name, method }, log) {
    return this.got.extend({
      method,
      hooks: {
        beforeRetry: [createOnRetry({ name, log })]
      },
      headers: {
  ...
  _logResponse ({
    ...
    const { statusCode, fromCache } = data

    const logProps = willLogResponseProps
      ? makeSafe(getLogResponseProps)(data)
      : {}

    const allLogProps = { ...logProps, statusCode, fromCache }
...
const createOnRetry = ({ name, log }) => (options, err, retryCount) => {
  log.warn({ err, isRetry: true, retryCount }, `${name}: Retried`)
}

const defaultGetHealth = async (client, path, baseUrl) => {
  const url = baseUrl === null ? {} : { baseUrl }
  await client.get(path, { json: false, ...url })
  return true
}
